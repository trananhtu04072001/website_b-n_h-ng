...
<div class="ordercus">
    <table>
        ...
        <tr>
                ...
                <td class="status">
                    @if ($val->status == 0)
                    <a href="{{route('order.status',$val->id)}}" class="btn btn-danger">Đặt lại</a>
                    @endif
                    @if ($val->status == 1)
                    <a href="#" class="btn btn-warning">Chờ xác nhận</a>
                    ...
                    @endif
                    @if ($val->status == 4)
                    <a href="#" class="btn btn-secondary">Đã Giao cho đvvc</a>
                    @endif
                    @if ($val->status == 5)
                    <a href="{{route('order.status',$val->id)}}" class="btn btn-success">Xác nhận đã nhận hàng</a>
                    <a href="#" class="btn btn-secondary">Trả lại hàng</a>
                    @endif
                    @if ($val->status == 6)
                ...
                <td>
                    {{-- <a href="{{route('order.detail',$val->id)}}" class="btn btn-secondary order_detail" name="order_detail" data-id="{{$val->id}}">Chi tiết</a> --}}
                    {{-- <a  class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#staticBackdrop">Chi tiết</a> --}}
